...
/**try { ... } catch (Exception ex) { ... }: This is a try-catch block. It attempts to perform a task (calculating the credit score) within the try block. If an error occurs, it catches the error and executes code within the catch block to handle the error gracefully.
